# Indirect call
間接呼叫（Indirect Call）是指在執行程式時，目標函式的位址並非直接寫死在指令中，而是透過暫存器或記憶體中的指標（Pointer）來跳轉。 

## 編譯
1. 編譯成 shared library（DLL）
    ```sh
...
objdump -d main
```
像是 `printf` 就對應到 `jmp    *0x2f56(%rip)`，因為他不是直接指向一個 address，所以是 indirect call
```
...
0000000000001060 <add@plt>:
    ...
    1064:       ff 25 5e 2f 00 00       jmp    *0x2f5e(%rip)        # 3fc8 <add@Base>
...
0000000000001169 <main>:
    ...
    1175:       be 04 00 00 00          mov    $0x4,%esi
    117a:       bf 03 00 00 00          mov    $0x3,%edi
